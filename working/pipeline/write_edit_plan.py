...
L = ["# Schnittplan – Immobilien-Walkthrough (~30 s)", "",
     ...
     f"Gesamtlänge geplant: **{sum(s['end_s']-s['start_s'] for s in plan['shots']):.1f} s** ({len(plan['shots'])} Shots, Übergänge: harte Schnitte mit {plan.get('xfade_s',0.2)} s Mini-Dissolve, 0,5 s Fade-in/-out).", "",
