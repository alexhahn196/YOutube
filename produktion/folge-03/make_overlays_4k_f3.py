#!/usr/bin/env python3
"""SIGNAL Ep03 overlays, nativ 4K (3840x2160): persistenter HUD (EP.03), Verdict-Meter
(Nadel 0.92 in NOISE, Frage: THE VIRAL CLAIMS), 7 Quellen-Bauchbinden + 1 Viral-Zaehler.
Timings aus VO-Segmentlaengen. Emits ov4k/*.png + overlays_4k.json."""
...
def make_hud():
    ...
    d.text((m+2*SC,m+46*SC),"EP.03 // VOYAGER 1 — THE FARTHEST MACHINE ALIVE",font=f(FM,15),fill=MUT)
    tag="TRK // V1 · 25 bn km · 2 instruments"
...
def make_meter():
    ...
    d.text((26*SC,16*SC),"VERDICT — THE VIRAL CLAIMS",font=f(FMB,17),fill=MUT)
...
# (segment_index 0-based, seconds_into_segment, source, detail, accent)
LT=[
 (0, 3,  "VIRAL WAVE — OWN SURVEY, 15 JUL 2026", "„Voyager 1 Turned Around…“ · 2,021,080 views in 6 days", RED),
 (2, 11, "GURNETT ET AL. · SCIENCE 2013", "plasma oscillations · crossing 25 Aug 2012 · 121.6 AU", CY),
 (2, 43, "RICHARDSON ET AL. · NATURE ASTRONOMY 2019", "30,000–50,000 K at the boundary · ~2× hotter than predicted", CY),
 (2, 70, "OCKER ET AL. · NATURE ASTRONOMY 2021", "persistent interstellar plasma hum · ~3 kHz · since 2017", CY),
 (3, 42, "NASA/JPL · 22 APR 2024", "„Voyager 1 Resumes Sending Engineering Updates to Earth“", CY),
 (6, 15, "PARK ET AL. · APJ 2026", "interstellar field still aligned · the 2015→2025 prediction failed", CY),
 (7, 15, "NASA VOYAGER BLOG · 17 APR 2026", "„NASA Shuts Off Instrument on Voyager 1“ · LECP off after 48 yrs", CY),
]
timings=[]
for i,(seg,sub,src,det,acc) in enumerate(LT):
    make_lt(i,src,det,acc)
    st=off[seg]+sub
    timings.append({"png":f"ov4k/lt_{i:02d}.png","start":round(st,2),"dur":6.0,"x":72*SC,"y":H-116*SC-70*SC})
# Meter in Seg 6 (0-based 5) bei "So. Signal, or noise?" ~ +93s
meter_start=off[5]+93
