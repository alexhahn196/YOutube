#!/usr/bin/env python3
"""Mix v4: loop 3 music beds per act, duck under VO (sidechain), mux with silent visual.
Output: f2_FINAL.mp4 (visuals + VO + scored music). No overlays yet."""
...
act=[sum(vslot[0:4]), sum(vslot[4:8]), sum(vslot[8:13])]
print("acts:", [round(a,1) for a in act], "total", round(sum(act),1))

beds=["music_act1.mp3","music_act2.mp3","music_act3.mp3"]
actwavs=[]
for i,(bed,adur) in enumerate(zip(beds,act)):
    bd=dur(os.path.join(HERE,bed)); loops=max(1,math.ceil(adur/bd)+1)
    o=os.path.join(WORK,f"actmus_{i}.wav")
    fade_out_st=max(0.1, adur-2.0)
    run([FF,"-y","-stream_loop",str(loops),"-i",os.path.join(HERE,bed),"-t",f"{adur:.3f}",
...
silent=os.path.join(HERE,"f2_video_silent.mp4"); vo=os.path.join(HERE,"f2_vo_full.wav")
final=os.path.join(HERE,"f2_FINAL.mp4")
run([FF,"-y","-i",silent,"-i",vo,"-i",music_full,"-filter_complex",
