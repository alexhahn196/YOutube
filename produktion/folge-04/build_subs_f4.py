#!/usr/bin/env python3
"""Approximate .srt fuer F4: Segment-Texte wie tts_f4.py re-derivieren, vo_seg-Dauern +
LEAD/GAP-Offsets, je Segment Satz-Cues, Zeit proportional nach Zeichenlaenge.
Segmentgrenzen exakt; innerhalb proportional."""
...
# --- replicate tts_f4.py segmentation ---
...
for ln in raw:
    ...
    s=re.sub(r"^\[LANGER BEAT\]$","…",s,flags=re.I); s=re.sub(r"^\[BEAT\b[^\]]*\]$","",s,flags=re.I)
    cur.append(s)
flush()
merged=[]
...
open(os.path.join(HERE,"signal_f4.srt"),"w").write("\n".join(out))
print(f"signal_f4.srt: {idx-1} cues, ends {tc(off)}")
