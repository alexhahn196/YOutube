#!/usr/bin/env python3
"""Composite SIGNAL Ep04 overlays onto f4_FINAL_4k -> f4_MASTER_4K.mp4 (2160p).
HUD persistent; timed lower-thirds/meter via 0-based fade + setpts PTS-shift.
Reads overlays_4k.json (native-4K PNGs). Audio copied from f4_FINAL_4k."""
import os, json, subprocess, re, imageio_ffmpeg
FF = imageio_ffmpeg.get_ffmpeg_exe()
HERE = os.path.dirname(os.path.abspath(__file__))
meta = json.load(open(os.path.join(HERE,"overlays_4k.json")))
VD = meta["video_dur"]
src = os.path.join(HERE,"f4_FINAL_4k.mp4"); out = os.path.join(HERE,"f4_MASTER_4K.mp4")
...
print("compositing 4K:", len(timed),"timed overlays + HUD -> f4_MASTER_4K.mp4")
...
print("f4_MASTER_4K done", d.group(1) if d else "?", res.group(1) if res else "?", os.path.getsize(out)//1048576,"MB")
